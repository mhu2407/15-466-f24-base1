...
## code is from Jude Markabawi's asset_gen.py with minor changes
## to load in other sprites in addition to the background
## Also collaborated with @choysal
...
def find_tiles_and_palettes(backgroundImg: np.ndarray, personImg: np.ndarray, sunscreenImg: np.ndarray):
    assert tuple(backgroundImg.shape) == bg_size, "Image must be 240x256x4 for this script"
    ## --- Tiles ---
    tiles = {}
    # loop through 8x8 pixel tiles in background
    for y in range(0, bg_size[0], 8):
        for x in range(0, bg_size[1], 8):
            tile = backgroundImg[y : y + 8, x : x + 8, :]
    ...
    # add other sprite to dict
    assert tuple(personImg.shape) == (8, 8, 4)
    assert tuple(sunscreenImg.shape) == (8, 8, 4)
    tiles[personImg.tobytes()] = personImg
    tiles[sunscreenImg.tobytes()] = sunscreenImg
    ...
    for y in range(0, bg_size[0], 8):
        for x in range(0, bg_size[1], 8):
            ...
            assert 0 <= tile_idx and tile_idx < (2 ** 8), "Too many tiles or I messed up somehwere"
            assert 0 <= palette_idx and palette_idx < (2 ** 3), "Too many tiles or I messed up somehwere"
    ...
    # insert palettes to empty table
...
# add to files to command
parser = ArgumentParser()
parser.add_argument("background")
parser.add_argument("person")
parser.add_argument("sunscreen")
args = parser.parse_args()

# load in all images
background = load_image(args.background)
person = load_image(args.person)
sunscreen = load_image(args.sunscreen)
find_tiles_and_palettes(background, person, sunscreen)
